...
// console.log(jonas.'last' + nameKey)          ->not work in dot notation
...
// Challenge - Jonas has 3 friends, and his best friend is called Michel

console.log(`${jonas.firstName} has ${jonas.friends.length},and his best friend is called ${jonas.friends[0]}`)

// // small break in this section debug code professional 
...
const tips=[];const totals=[];for(let i=0;i<bills.length;i++){const tip=calcTip(bills[i]);tips.push(tip);totals.push(bills[i]+tip);totals.push(bills[i]+tip)}
function calcAverage(arr){let sum=0;for(let i=0;i<arr.length;i++){sum+=arr[i]}return sum/arr.length;}