...
const getCountryData = function(country){
  ...
  .then(data => {
    ...
    const nextNeighbour= data.borders?.[0];
    if(!nextNeighbour) return;

    return fetch(`https://restcountries.com/v2/alpha/${nextNeighbour}`)
  })
  ...
  .then(data=>{
    if(!data) return;
    renderCountry(data,'next-neighbour')
  })
...
//////////////////////////////////////////////////////////////////
// Returning Valus from Async Function
...
const whereAmI=async function(){
  try{
    ...
    // Reverse geocoding
    const resGeo=await fetch(`https://api.bigdataloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}`)
